...
{% hint style="danger" %}
**Animarr is currently in BETA.** If you would like to help test the bleeding edge, please use the image **`sctx/animarr:develop`**!
{% endhint %}

{% hint style="info" %}
After running Animarr for the first time, configure it by visiting the web UI at `http://[address]:5055` and completing the setup steps.
{% endhint %}

## Docker

{% hint style="warning" %}
Be sure to replace `/path/to/appdata/config` in the below examples with a valid host directory path. If this volume mount is not configured correctly, your Animarr settings/data will not be persisted when the container is recreated (e.g., when updating the image or rebooting your machine).
...
```bash
docker stop animarr && docker rm animarr
```

Pull the latest image:

```bash
docker pull sctx/animarr
```
...
{% hint style="info" %}
You may alternatively use a third-party updating mechanism, such as [Watchtower](https://github.com/containrrr/watchtower) or [Ouroboros](https://github.com/pyouroboros/ouroboros), to keep Animarr up-to-date automatically.
{% endhint %}
...
Define the `animarr` service in your `docker-compose.yml` as follows:
...
2. Inside the **Community Applications** app store, search for **Animarr**.
3. Click the **Install Button**.
4. On the following **Add Container** screen, make changes to the **Host Port** and **Host Path 1**\(Appdata\) as needed.
5. Click apply and access "Animarr" at your `<ServerIP:HostPort>` in a web browser.
...
First, create a volume to store the configuration data for Animarr using using either the Docker CLI:

```bash
docker volume create animarr-data
```
...
4. Enter a name for the volume (example: `animarr-data`) and hit "Create"

Then, create and start the Animarr container:

```bash
docker run -d --name animarr -e LOG_LEVEL=debug -e TZ=Asia/Tokyo -p 5055:5055 -v "animarr-data:/app/config" --restart unless-stopped sctx/animarr:latest
```
...
To access the files inside the volume created above, navigate to `\\wsl$\docker-desktop-data\version-pack-data\community\docker\volumes\animarr-data\_data` using File Explorer.
...
{% hint style="info" %}
The [Animarr snap](https://snapcraft.io/animarr) is the only officially supported Linux install method aside from [Docker](#docker).
...
```
sudo snap install animarr
```

{% hint style="danger" %}
To install the development build, add the `--edge` argument to the above command (i.e., `sudo snap install animarr --edge`). However, note that this version can break any moment. Be prepared to troubleshoot any issues that arise!
{% endhint %}

**Updating:**

Snap will keep Animarr up-to-date automatically. You can force a refresh by using the following command.
...
{% tab title="Gentoo" %}
Portage overlay [GitHub Repository](https://github.com/chriscpritchard/animarr-overlay).
...
```bash
eselect repository enable animarr-overlay
```
...
```bash
emerge www-apps/animarr
```

**To install the development build:**
A live ebuild (`=www-apps/animarr-9999`) is also available. To use this, you will need to modify accept_keywords for this package:

```bash
emerge --autounmask --autounmask-write "=www-apps/animarr-9999"
```
...
The installation is not implemented via Docker, but barebones. The latest release version of Animarr will be used.
Please see the [swizzin documentation](https://swizzin.ltd/applications/animarr) for more information.

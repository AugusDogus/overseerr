...
const OLD_LOG_FILE = path.join(__dirname, '../config/logs/animarr.log');
  ...
  transports: [
    ...
    new winston.transports.DailyRotateFile({
      filename: process.env.CONFIG_DIRECTORY
        ? `${process.env.CONFIG_DIRECTORY}/logs/animarr-%DATE%.log`
        : path.join(__dirname, '../config/logs/animarr-%DATE%.log'),
      datePattern: 'YYYY-MM-DD',
      zippedArchive: true,
      maxSize: '20m',
      maxFiles: '7d',
      createSymlink: true,
      symlinkName: 'animarr.log',
    }),
    new winston.transports.DailyRotateFile({
      filename: process.env.CONFIG_DIRECTORY
